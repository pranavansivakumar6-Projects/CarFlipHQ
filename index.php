...
$overdueTasks = $pdo->query("SELECT COUNT(*) FROM tasks WHERE status != 'Done' AND due_date IS NOT NULL AND due_date < CURDATE()")->fetchColumn();
$readyCars = $pdo->query("SELECT COUNT(*) FROM cars WHERE status IN ('Ready for Sale','Listed')")->fetchColumn();
...
$expectedSales = $pdo->query("SELECT COALESCE(SUM(estimated_sale_price),0) FROM cars WHERE status != 'Sold'")->fetchColumn();
$expectedProfit = $expectedSales - $totalPurchase - $totalExpenses;
?>
<div class="container">
    <h1>Dashboard</h1>
    <div class="grid">
        ...
        <div class="card"><div>Expected Active Profit</div><div class="profit <?= $expectedProfit >= 0 ? 'positive' : 'negative' ?>">$<?= number_format($expectedProfit, 2) ?></div></div>
    </div>
